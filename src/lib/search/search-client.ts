...
class SearchController {
  ...
  private renderResults() {
    ...
    if (this.el.count) {
      this.el.count.textContent = `${this.results.length} resultados`;
    }
    ...
    this.el.list!.innerHTML = this.results
      ...
      >
        ...
        ${
          r.meta.type || r.meta.category || r.meta.complexity
            ? `<div class="mt-1 flex flex-wrap gap-1">${[
                r.meta.type && `<span class="rounded-full bg-muted px-2 py-0.5 text-[10px] uppercase">${this.esc(r.meta.type)}</span>`,
                r.meta.category && `<span class="rounded-full bg-muted px-2 py-0.5 text-[10px] uppercase">${this.esc(r.meta.category)}</span>`,
                r.meta.complexity && `<span class="rounded-full bg-muted px-2 py-0.5 text-[10px] uppercase">${this.esc(r.meta.complexity)}</span>`,
  ...
  private renderEmpty() {
    ...
    this.el.empty!.innerHTML = `<p class="text-muted-foreground">Sin resultados para "${this.esc(this.query)}"</p>`;
